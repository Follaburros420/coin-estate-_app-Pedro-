  ...
  return (
    <div className=' max-w-[1192px] mx-auto w-full  bg-white px-4 '>
      <div className='flex fixed top-0 left-0 right-0 bg-white z-50 items-center w-full justify-between sm:hidden p-4 '>
        <button onClick={() => router.push('/')} className='w-full max-w-[153px] '>
          <StyledImage src='/assets/svg/coinEstateLogo.svg' className='w-full max-w-[153px] h-9 ' alt='' />
        </button>
        <button onClick={() => setIsOpen(!isOpen)} className='bg-grey-1100 p-1 rounded-sm focus:outline-none '>
          ...
          {Navbar_Links.map((item, idx) => {
            return (
              <Link
                key={idx}
                href={item.path}
                className={clsxm(path === item.path ? 'text-yellow ' : 'text-black-100')}>
                {item.title}
              </Link>
            );
          })}
