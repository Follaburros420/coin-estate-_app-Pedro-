    ...
    <div className='px-6 '>
      <div className=' mt-16 lg:mt-[230px] max-w-[1161px] mx-auto w-full '>
        <p className='text-center text-black-100 text-28 lg:text-36 font-bold font-inter'>
          ¿Qué hacemos en <span className='text-yellow'>CoinEstate</span>?
        </p>
        <p className='text-14 lg:text-20 font-inter font-regular text-black-100 max-w-[919px] mx-auto text-center '>
          Facilitamos el acceso a la inversión en proyectos inmobiliarios de los Estados Unidos a través de la
            ...
            return (
              <div key={`${items.id}____${idx}`}>
                <div className='max-w-[248px] mx-auto lg:mx-0 p-4'>
                  ...
                  <p className='mt-6 text-14 font-regular font-inter text-gray'>{items.text}</p>
                </div>
              </div>
            );
          })}
        ...
        <div className='w-full my-10'>
          <button onClick={()=>router.push('/marketplace')} className='w-full mt-4 max-w-[146px] mx-auto flex items-center justify-center text-center p-3 rounded-full bg-Yellow-200 text-white font-bold text-14  '>
            Ver MarketPlace
          </button>
        </div>
      </div>
